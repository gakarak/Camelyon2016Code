...
% %     fnProb=[fnTifBase,'_MaskTiles/idx-tiles.txt-GoogleNet-Prob.csv'];
    fnProb=[fnTifBase,'_MaskTiles/idx-tiles.txt_HIST_q512_LSQ1_k100-ProbDst.csv'];
...
% %     mskVis=imresize(imread(fnTifMskVis),sizMsk);
    ...
    subplot(1,2,1), imshow(mskProb>0.99,[]), title('Prob-Map');
    subplot(1,2,2), imshow(mskDist,[]), title('Prob-Dist');
