...
% %     timgDsc=reshape(timgd,[],3);
    ...
    numTum=sum(tevalV>0);
    if numTum>numMaxSamplesFromSlice
        numTum=numMaxSamplesFromSlice;
    end
    tmpArrDscTumor=timgDsc(tevalV>0,:);
    %
    mskNormT0=(rgb2gray(timg)>valThreshI);
    mskNormT=imopen(mskNormT0,se);
    mskNormComb=(timgEvalDil<1)&(mskNormT);
    mskNormCombV=reshape(mskNormComb,[],1);
    tmpArrDscNormalTot=timgDsc(mskNormCombV>0,:);
    %
% % %     subplot(2,2,1),
% % %     imshow( imfuse(mskNormT, (timgEvalDil<1)) );
% % %     subplot(2,2,2), imshow(mskNormT);
% % %     subplot(2,2,3), imshow(mskNormComb)
    %
