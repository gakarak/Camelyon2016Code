...
######################################
def usage(sysArgv):
    print 'Usage: %s {/path/dir/with/caffe.model} {/path/to/filenameas-and-labels-index.txt}' % os.path.basename(sysArgv[0])
    ...
    def processBatchFromTiff(self, ptrTiff, lstRows, lstCols, idLevel=2, sizTile=64):
        ...
        for bbi,bb in enumerate(lstBatches):
            ...
            for ppi,pp in enumerate(bb):
                ...
                # timg0=np.array(ptrTiff.dataImg.read_region((tcol0-4*siz2/2,trow0-4*siz2/2),0, (siz2*4,siz2*4)))[:,:,:3]
                timgr=255.*tf.resize(timg, (sizTile,sizTile), order=3)
                # plt.figure()
                # plt.subplot(1,2,1)
                # plt.imshow(timg)
                # plt.subplot(1,2,2)
...
if __name__=='__main__':
    ...
    parser.add_argument("-pathImg",  help="Path to BigTiff Image", required=True)
    parser.add_argument("-pathMsk",  help="Path to Mask Image", required=True)
    opt=parser.parse_args()
    print '-------'
    checkFile(opt.dirCaffe, isCheckDir=True)
    checkFile(opt.pathImg)
    checkFile(opt.pathMsk)
    print '-------'
    tiffReader=BigTiffReader(opt.pathImg)
    print tiffReader
    xmlInfo=bsoup(tiffReader.dataImg.properties['tiff.ImageDescription'],'lxml-xml')
    imgMsk=(io.imread(opt.pathMsk)<1)
    imgMsk=morph.dilation(imgMsk, selem=disk(30))
    ridx,cidx=np.where(imgMsk)
    #
    # ridx=ridx[:1000]
    # cidx=cidx[:1000]
    #
    nrow,ncol=imgMsk.shape
    l2ncol,l2nrow = tiffReader.layerSizes[0]
    l2rows=np.round(l2nrow * (ridx + 0.5) / nrow)
    l2cols=np.round(l2ncol * (cidx + 0.5) / ncol)
    #
    clsCaffe=CaffeBatchClassifier()
    clsCaffe.loadModelFromDir(opt.dirCaffe)
    print clsCaffe
    #
    retProb=clsCaffe.processBatchFromTiff(tiffReader, l2rows, l2cols, sizTile=tlsiz)
    # retProb=ret[0]
    imgProb=np.zeros(imgMsk.shape)
    imgProb[ridx,cidx]=retProb
    foutMsk='%s-clsmsk.png' % opt.pathMsk
    imgProbu8=np.uint8(255.*imgProb)
    print "fuck: max=", np.max(imgProb)
    io.imsave(foutMsk, imgProbu8)
    #
    plt.figure()
    plt.subplot(1,2,1)
    plt.imshow(imgMsk, cmap=plt.gray())
    plt.subplot(1,2,2)
    plt.imshow(imgProb)
    plt.show()
    print '---------'
