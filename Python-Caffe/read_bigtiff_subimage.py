...
class BigTiffReader:
    ...
    def loadImage(self, parFnImg):
        if os.path.isfile(parFnImg):
            self.dataImg=ops.OpenSlide(fnImg)
...
if __name__=='__main__':
    fnImg='/home/ar/data/Camelyon16_Challenge/data/Tumor_001.tif'
    dataImg=BigTiffReader(fnImg)
    print dataImg
    #
    idxLevel=dataImg.numLevels-3
    # timg=np.array(dataImg.dataImg.read_region((0,0),idxLevel, dataImg.layerSizes[idxLevel]))[:,:,:3]
    timg=dataImg.getImageOnLevel(idxLevel)
    plt.imshow(timg)
    plt.show()
